/*O custo de um carro novo ao consumidor é a soma do custo de fábrica com a porcentagem do 
distribuidor e dos impostos (aplicados ao custo de fábrica). Supondo que o percentual do distribuidor seja de 28% e os impostos de 45%, escrever um algoritmo para ler o custo de fábrica de um carro, calcular e escrever o custo final ao consumidor.*/

let custoCarroFabrica = parseFloat(prompt("informe o custo de fabrica do carro:"));
const distribuidor = 28 / 100;
const imposto = 45 / 100;

let carroConsumidor = custoCarroFabrica + (distribuidor * custoCarroFabrica) + (imposto * custoCarroFabrica);

alert("O preço do carro é: " + carroConsumidor);